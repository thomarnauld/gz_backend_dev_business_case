...
export class DeliveryService {
  ...
  getAll(): Observable<Delivery[]> {
    return this.http.get<Delivery[]>('http://localhost:3000/api/delivery');
  }

  addDelivery(searchId: string, data: Delivery): Observable<Delivery> {
  ...
  delDelivery(searchId: string): Observable<Delivery> {
    return this.http.delete<Delivery>(
      'http://localhost:3000/api/delivery/' + searchId
    );
  }
}
